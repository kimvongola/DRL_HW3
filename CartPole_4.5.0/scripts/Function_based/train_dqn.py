...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    while simulation_app.is_running():
        # with torch.no_grad():  # avoids tracking gradients during inference
            ...
            for episode in tqdm(range(n_episodes)):
                ...
                while not done:
                    ...
                    action_tensor, action_idx = agent.select_action(state)
                    ...
                    # --- Store transition ---
                    agent.memory.add(
                        obs['policy'],           # convert back to numpy
                        action_idx,
                        reward_value,
                        next_obs['policy'],
                        terminated.item()
                ...
                # --- Print + Save ---
                if episode % 100 == 0:
                    print(f"Episode {episode} | Avg Reward: {sum_reward / 100:.2f} | Epsilon: {agent.epsilon:.3f}")
                    sum_reward = 0
